...
int main(int argc, char *argv[]){
  if( argc==1 )
    printf("\nPng2Scr v1.20. Image to ZX Spectrum SCR screen by AntonioVillena, 7 Jul 2014\n\n"
           "  Png2Scr <input_png> <output_scr> [output_attr] [-i] [-a input_attr]\n\n"
  ...
  for ( i= 0; i < height>>6; i++ )
    for ( j= 0; j < 0x100; j++ ){
      ...
      if( input2 )
        tinta= input[i<<8 | j]&7 | input[i<<8 | j]>>3&8,
        fondo= input[i<<8 | j]>>3&15;
      else
